...
@signal_base.function
def interp_orf(pos1, pos2, params, diag=1.):
    '''
    ...
    Changing angles will require manual intervention to create new function.
...
@signal_base.function
def bin_cos_orf(pos1, pos2, params, diag=1.):
    '''
    Agnostic binned spatial correlation function. Bin edges are
...
def anis_orf(pos1, pos2, params, **kwargs):
    ...
    if lmax > 0:
        #clm[1:] = params[1:]
        clm[1:] = params
    clm_basis = [clm[ii] * basis for ii, basis in enumerate(anis_basis[: (lmax + 1) ** 2, psr1_index, psr2_index])]
    
    return sum(clm[ii] * basis for ii, basis in enumerate(anis_basis[: (lmax + 1) ** 2, psr1_index, psr2_index]))


@signal_base.function
def pixelmap_orf(pos1, pos2, matrix = None, psrspos = None):
    psr1_index = [ii for ii in range(len(psrspos)) if np.all(psrspos[ii] == pos1)][0]
    psr2_index = [ii for ii in range(len(psrspos)) if np.all(psrspos[ii] == pos2)][0]

    value = matrix[psr1_index][psr2_index]
    
    return value


@signal_base.function
def test_gworf(pos1, pos2, **kwargs):
    ...
    dec1=np.arcsin(pos1[2])
    ra1 = np.arctan2(pos1[1],pos1[0])

    if (ra1 < 0):
        ra1+=2*np.pi

    dec2=np.arcsin(pos2[2])
    ra2 = np.arctan2(pos2[1],pos2[0])

    if(ra2 <0 ):
        ra2 += 2*np.pi

    ptapsrs.append(gworf.psr(raj=ra1, decj=dec1, dist=1))
    ptapsrs.append(gworf.psr(raj=ra2, decj=dec2, dist=1.5))


    F_e=gworf.signalResponse_earth(ptapsrs,pixels[0][:],pixels[1][:], dirconv=False)    

    # isotropic sky map
    sky_pixel=np.zeros(hp.nside2npix(nside))
    sky_pixel[pixel]=1

    # read in custom sky map
    skypixel = kwargs['pmap']/np.amax(kwargs['pmap'])
    sky_pixel[pixel]=1

    orf_iso = 0.5 * gworf.orfFromMap_fast(usermap=skypixel, response=F_e).real

    # this contains the cross correlation function
    rho=orf_iso.real[0][1]
