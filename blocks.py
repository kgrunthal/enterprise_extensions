...
def white_noise_block(vary=False, inc_ecorr=False, gp_ecorr=False,
                      ...
                      ecorr_select='nanograv', common=False, name=None):
    """
...
def red_noise_block(psd='powerlaw', prior='log-uniform', Tspan=None,
                    name='red_noise', components=30, logf=False,
                    fmin=None, fmax=None, tnfreq=False,
                    ...
                    logmin=None, logmax=None, dropout=False,
                    dropbin=False, dropbin_min=10, k_threshold=0.5):
    """
        ...
        if psd == 'powerlaw':
            if any([dropout, dropbin]):
                ...
                if dropbin:
                    ...
                    k_dropbin = None
                pl = drop.dropout_powerlaw(log10_A=log10_A, gamma=gamma, k_drop=k_drop,
                                           k_dropbin=k_dropbin, k_threshold=k_threshold)
            else:
...
def dm_noise_block(gp_kernel='diag', psd='powerlaw', nondiag_kernel='periodic',
                   ...
                   logf=False, fmin=None, fmax=None, components=30,
                   tnfreq=False, select=None, modes=None,
                   gamma_val=None, delta_val=None, coefficients=False,
...
def common_red_noise_block(psd='powerlaw', prior='log-uniform',
    ...
    orfs = {'crn': None, 'crn_chrom': None, 'hd': model_orfs.hd_orf(),
            ...
            'pixelmap_orf': model_orfs.pixelmap_orf(matrix = orf_matrix, psrspos = psrs_pos)
             }
    ...
    elif isinstance(orf, type(model_orfs.hd_orf())):
